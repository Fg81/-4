...
export default function Admin() {
  ...
  const { data: configurations = [] } = useQuery({
    queryKey: ["/api/audio/configurations"],
    enabled: !!admin,
  });
  ...
  const deleteConfigurationMutation = useMutation({
    mutationFn: async (id: string) => {
      await apiRequest("DELETE", `/api/audio/configurations/${id}`);
    },
    onSuccess: () => {
      toast({
        title: "Успешно",
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/audio/configurations"] });
    },
    onError: () => {
      toast({
        title: "Ошибка",
          ...
          <main className="flex-1 p-6 overflow-auto">
            ...
            <Tabs defaultValue="articles" className="w-full">
              <TabsList className="grid w-full grid-cols-6">
                ...
                <TabsTrigger value="configurations">Конфигурации</TabsTrigger>
              ...
              <TabsContent value="configurations" className="space-y-6">
                <Card>
                  <CardHeader>
                    <CardTitle>Управление конфигурациями</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {(configurations as any[]).length > 0 ? (
                        (configurations as any[]).map((config: any) => (
                          <div key={config.id} className="flex items-center justify-between p-4 border rounded-lg">
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-muted-foreground">
                                Автомобиль: {config.carModel} • Бюджет: {config.budget?.toLocaleString()}₽
                              </div>
                              <div className="text-sm text-muted-foreground">
                                Приоритет: {
                                  config.priority === 'sound_quality' ? 'Качество звука' :
                                  config.priority === 'bass' ? 'Мощный бас' :
                                  config.priority === 'volume' ? 'Максимальная громкость' : 'Сбалансированность'
                                }
                              </div>
                              <div className="text-sm text-muted-foreground">
                                Создано: {new Date(config.createdAt).toLocaleDateString('ru-RU')}
                              </div>
                              {config.musicGenres && (
                                <div className="text-sm text-muted-foreground">
                                  Жанры: {JSON.parse(config.musicGenres).join(", ")}
                                </div>
                              )}
                            </div>
                            <div className="space-x-2">
                              <Button
                                ...
                                onClick={() => {
                                  try {
                                    const configData = JSON.parse(config.configurationData);
                                    console.log("Configuration details:", configData);
                                    toast({
                                      title: "Подробности конфигурации",
                                      description: "Детали выведены в консоль браузера",
                                    });
                                  } catch (e) {
                                    toast({
                                      title: "Ошибка",
                                      description: "Не удалось прочитать данные конфигурации",
                                      variant: "destructive",
                                    });
                                  }
                                }}
                        ...
                        <div className="text-center py-8">
                          <p className="text-muted-foreground">Конфигурации не найдены</p>
                          <p className="text-sm text-muted-foreground mt-2">
                            Конфигурации будут появляться здесь после их создания в конфигураторе автозвука
                          </p>
                        </div>
                      )}
                    </div>
